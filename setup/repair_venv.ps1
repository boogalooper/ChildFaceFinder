...
function Test-NativeCommand {
    ...
    try {
        ...
        # NativeCommandError while ErrorActionPreference is Stop. A broken uv
        # trampoline must be treated as a normal failed validation instead.
...
if (-not $IsRelocatable -or -not $VenvWorks) {
    ...
    $env:UV_NO_CONFIG = '1'
    try {
        # --allow-existing preserves site-packages. --relocatable regenerates
        # Windows python/pythonw trampolines with relocatable targets instead
        # of merely editing pyvenv.cfg, which is insufficient after a move.
